...
class ElementNameTests {
	...
	class MainNameTests implements NameTester, QualifiableTester {
		...
		@Test
		void test() {
			assertThat(subject.qualifiedBy(Qualifiers.of("foo")), isA(Qualifier.class));
			assertThat(ElementName.configurationName("linkElements").qualifiedBy(subject.qualifiedBy(Qualifiers.of("foo"))), hasToString("fooLinkElements"));
		}
	}
	...
	class OtherNameTests implements NameTester, QualifiableTester {
		...
		void test() {
			assertThat(subject.qualifiedBy(Qualifiers.of("foo")), isA(Qualifier.class));
			assertThat(ElementName.configurationName("linkElements").qualifiedBy(subject.qualifiedBy(Qualifiers.of("foo"))), hasToString("fooSharedLibraryLinkElements"));
		}
	}
	...
	class VerbOnlyTaskNameTests implements NameTester, QualifiableTester {
		...
		void qualifiedNameAreNotQualifier() {
			assertThat(subject.qualifiedBy(Qualifiers.of("foo")), not(isA(Qualifier.class)));
		}
	}
